@if (isset($edit))
    <form action="{{ route('member-alloted-leave.update', $allotedLeave->id) }}" method="POST"
        id="member-alloted-leave-edit-form">
        @method('PUT')
        @csrf
                                ...
                                <select class="form-select" name="member_id" id="member_id" disabled>
                                    ...
                                    @foreach ($members as $member)
                                        <option value="{{ $member->id }}"
                                            {{ $member->id == $allotedLeave->member_id ? 'selected' : '' }}>
                                            {{ $member->name }}</option>
                                    @endforeach
                                </select>
                                ...
                                <select class="form-select" name="leave_type_id" id="leave_type_id">
                                    ...
                                    @foreach ($leaveTypes as $leaveType)
                                        <option value="{{ $leaveType->id }}"
                                            {{ $leaveType->id == $allotedLeave->leave_type_id ? 'selected' : '' }}>
                        ...
                        <div class="row align-items-center">
                            ...
                            <div class="col-md-12">
                                <input type="text" class="form-control" name="alloted_leaves" id="alloted_leaves"
                                    value="{{ $allotedLeave->alloted_leaves }}">
                                <span class="text-danger"></span>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="col-md-12">
                                <select class="form-select" name="year" id="year" disabled>
                                    ...
                                    @foreach ($years as $year)
                                        <option value="{{ $year }}"
                                            {{ $year == $allotedLeave->year ? 'selected' : '' }}>{{ $year }}
                                        </option>
                                    @endforeach
                                </select>
                                <span class="text-danger"></span>
                        ...
                        <div class="row align-items-center">
                            ...
                            <div class="col-md-12">
                                <select class="form-select" name="status" id="status">
                                    ...
                                    <option value="1" {{ $allotedLeave->status == '1' ? 'selected' : '' }}>
                                        Active</option>
                                    <option value="0" {{ $allotedLeave->status == '0' ? 'selected' : '' }}>
                                        Inactive</option>
                                </select>
                                <span class="text-danger"></span>
                            </div>
