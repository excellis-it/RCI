@if (count($inventoryProjects) > 0)
    @foreach ($inventoryProjects as $key => $inventoryProject)
        <tr>
            ...
            <td>{{ $inventoryProject->project_code ?? 'N/A' }}</td>
            <td><span
                    class="{{ $inventoryProject->status == 1 ? 'active_ss' : 'inactive_ss' }}">{{ $inventoryProject->status == 1 ? 'Active' : 'Inactive' }}</span>
            </td>
            <td class="sepharate"><a data-route="{{ route('inventory-projects.edit', $inventoryProject->id) }}"
    ...
    <tr class="toxic">
        <td colspan="7" class="text-left">
            <div class="d-flex justify-content-between">
                <div class="">
                    (Showing {{ $inventoryProjects->firstItem() }} – {{ $inventoryProjects->lastItem() }} Inventory
                    Projects of
